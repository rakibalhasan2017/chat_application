...
export const getuser = async (req, res) => {
  return res.status(200).json({
    id: req.user._id,
    profilepic: req.user.profilepic, // Assuming profilepic is stored in the user's model
  });
};
...
export const updateprofile = async (req, res) => {
  try {
    ...
    if (!profilepic) {
      return res.status(400).send("profile picture must be required");
      console.log("profile picture must be required");
    }
    console.log("heda print kor");
